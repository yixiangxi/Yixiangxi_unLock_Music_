...
namespace UnLockMusic
{
    class clsMusicOperation
    {
        ...
        /// <returns> lmsc  音乐列表</returns>
        public List<clsMusic> GetMusicList(string SongName)
        {
            List<clsMusic> lmsc = new List<clsMusic>();
            var list = Parallel(() => GetWYYMusicList(SongName), () => GetKWMusicList(SongName), () => GetKGMusicList(SongName),  () => GetQQMusicList(SongName));
        ...
        public List<clsMusic> GetKWMusicList(string SongName)
        {
            ...
            try
            {
                ...
                Console.WriteLine($"JSON Exception occurred: {ex.Message}");
            }
            catch (Exception ex)
            {
        ...
        public string GetMusicDownloadURL(string date, enmMusicSource emsSource = enmMusicSource.QQ)
        {
            ...
            switch (emsSource)
            {
                ...
                case enmMusicSource.Kg:
                    url = m_strGetKgMusicDownloadURL.Replace("<<FileHash>>", date);
                    //.Replace("<<KgMid>>", "c596eb268a2705383a10d0af021664c0");//.Replace("<<KgDFID>>","07u9ob41Vu350chwOw4ejU7b");
                    ResultURL = m_htpClient.GetWeb(url);
                    ...
                    if (ResultURL.Substring(1, 10) == "\"status\":1")
                    {
                        ...
                        m_strFileFormat = ResultURL.Substring(ResultURL.Length - 4);
                        //可能会出错，如果后缀不是3个字节的话
                    }
                    break;
